...
double drwnOptimizer::EPSF = 1.0e-6;
double drwnOptimizer::EPSG = 1.0e-3;
double drwnOptimizer::EPSX = 1.0e-6;
...
double drwnOptimizer::solve(unsigned maxiter, bool bMonitor)
{
    DRWN_ASSERT(_x != NULL);
    ...
    drwnLBFGSResult info = lbfgsMinimize(m, maxiter, EPSG, EPSF, EPSX, bMonitor);
    switch (info) {
    case DRWN_LBFGS_CONVERGED_F:
        DRWN_LOG_VERBOSE("Optimization converged (relative function decrease <= " << EPSF << ")");
        break;
    case DRWN_LBFGS_CONVERGED_X:
        DRWN_LOG_VERBOSE("Optimization converged (step size <= " << EPSX << ")");
        break;
    case DRWN_LBFGS_CONVERGED_G:
        DRWN_LOG_VERBOSE("Optimization converged (gradient norm <= " << EPSG << ")");
        break;
    case DRWN_LBFGS_MAX_ITERS:
        DRWN_LOG_VERBOSE("Maximum number of iterations reached.");
        break;
    default:
    ...
    double epsg, double epsf, double epsx, bool bMonitor)
{
    ...
    DRWN_ASSERT((epsg >= std::numeric_limits<double>::min()) && (epsf >= 0.0) && (epsx >= 0.0));
