...
class drwnOptimizer {
 public:
    static double EPSF; //!< default tolerance on function convergence
    static double EPSG; //!< deafult tolerance on gradient convergence
    static double EPSX; //!< default tolerance on solution convergence
 ...
 public:
    ...
    //! Solve the optimization problem for up to \b maxiter iterations to precision set
    //! by \b EPSF, \b EPSG, and \b EPSX static variables. Calls \ref monitor function
    //! after each iteration if \b bMonitor is \a true.
