...
drwnBoostedClassifierNode::drwnBoostedClassifierNode(const char *name, drwnGraph *owner) :
    drwnNode(name, owner), _trainingColour(-1), _subSamplingRate(0)
{
    _nVersion = 100;
    _desc = "Boosted decision tree classifier";
...
drwnBoostedClassifierNode::drwnBoostedClassifierNode(const drwnBoostedClassifierNode& node) :
    drwnNode(node), _trainingColour(node._trainingColour),
    _subSamplingRate(node._subSamplingRate), _classifier(node._classifier)
{
    // declare propertys
    declareProperty("trainingColour", new drwnIntegerProperty(&_trainingColour));
    declareProperty("subSample", new drwnIntegerProperty(&_subSamplingRate));
...
drwnBoostedClassifierNode::~drwnBoostedClassifierNode()
{
    // do nothing
}

// i/o
bool drwnBoostedClassifierNode::save(drwnXMLNode& xml) const
{
    drwnNode::save(xml);
    drwnXMLNode *node = drwnAddXMLChildNode(xml, "drwnClassifier", NULL, false);
...
void drwnBoostedClassifierNode::evaluateForwards()
{
...
void drwnBoostedClassifierNode::updateForwards()
{
...
void drwnBoostedClassifierNode::resetParameters()
{
    // clear model
    _classifier.initialize(0, 0);
}

void drwnBoostedClassifierNode::initializeParameters()
{
    DRWN_FCN_TIC;
    ...
    resetParameters();
    if (_classifier.getIntProperty(_classifier.findProperty("numRounds")) < 1) return;
...
DRWN_AUTOREGISTERNODE("Adaptive", drwnBoostedClassifierNode);
