% BUILDMACOSX   Script for compiled mex interface to Darwin for Mac OS X
...
if (exist('../../external/opencv', 'dir')),
    [status, mexOpenCVOptions] = system('/usr/local/bin/pkg-config --cflags --libs ../../external/opencv/lib/pkgconfig/opencv.pc');
    if (status ~= 0),
        error(['ERROR: ', mexOpenCVOptions]);
    end;
    
    mexVisionBuildOptions = ['-D__LINUX__ -D__APPLE__ -I../../external -I../../include -L../../bin ', ...
